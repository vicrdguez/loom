...
deepened module is tested across its seam. (In Loom, this same taxonomy **is** the mocking decision
in `loom-apply` — see `loom-apply/reference/mocking.md`.)
...
Dependencies that have local test stand-ins (PGLite for Postgres, in-memory filesystem, Ecto
sandbox). Deepenable if the stand-in exists. The deepened module is tested with the stand-in running
in the test suite. The seam is internal; no port at the module's external interface.
...
Tests use an in-memory adapter; production uses an HTTP/gRPC/queue adapter.
...
Third-party services (Stripe, Twilio, etc.) you don't control. The deepened module takes the external
...
- Old unit tests on **shallow** modules become waste once tests at the deepened module's interface
  exist — delete them. (This applies to *pass-through* shallows, not to substantial internal modules
  that carry their own invariants — those keep their own tests at their own seam.)
